	...
	public function index() {
		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				if ($result['image']) {
                    $image = $this->model_tool_image->resize($result['image'], 366, 250);
                } else {
			...
			foreach ($results as $result) {
				...
				$name_products = html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8');
			...
			$limits = array_unique(array($this->config->get('theme_' . $this->config->get('config_theme') . '_product_limit'), 50, 100));
