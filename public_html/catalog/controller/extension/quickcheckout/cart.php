	...
	public function index() {
		...
		foreach ($products as $product) {
			...
			foreach ($discounts as $discount) {
				...
				$skidka = round(100-((int)$discount['price']/((int)$price/100)));
				$discountss[] = array(
					'quantityp' => $product['quantity'],
					'pricep' 	=> $price,
					'quantitys' 	=> $discount['quantity'],
					'price'    	=> $skidka
				);
				  
			}   
			
			$name_products = html_entity_decode($product['name'], ENT_QUOTES, 'UTF-8');
