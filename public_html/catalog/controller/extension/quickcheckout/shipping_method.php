...
class ControllerExtensionQuickCheckoutShippingMethod extends Controller {
  	public function index() {
		...
		$data['mapsyandex'] = html_entity_decode($this->config->get('shipping_free_maps'), ENT_QUOTES);
		...
		if( $this->cart->getSubTotal() < 1500 ) {
			$diff = (float)(1500 - $this->cart->getTotal());
		} else {
			$diff = (float)($this->cart->getTotal() - 1500);
		}
 
		if ($this->cart->getSubTotal() < 1500) {
 
			$data['alert'] = sprintf( $this->language->get('text_alert'), $this->currency->format($this->tax->calculate($diff, $this->config->get('config_tax')), $this->session->data['currency']));
