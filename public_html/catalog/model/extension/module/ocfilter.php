...
class ModelExtensionModuleOCFilter extends Model {
  ...
  public function getManufacturersByCategoryId($category_id) {
	...
	public function getWeightByCategoryId($category_id) {
		...
		$sql = "SELECT p.weight AS value_id, p.weight, 'w' AS option_id, p.weight_class_id AS weight_class_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id)";
		...
		$sql .= " GROUP BY p.weight ORDER BY p.weight ASC";
  ...
  public function getSearchSQL($params = array()) {
	...
	$union[] = $this->getWeightCounterSQL($data);
  ...
  private function getManufacturersCounterSQL($data) {
	...
	private function getWeightCounterSQL($data) {
		if (!empty($data['filter_ocfilter'])) {
			$params = $this->decodeParamsFromString($data['filter_ocfilter']);
		...
		$sql = "SELECT p.weight AS value_id, p.weight, 'w' AS option_id, COUNT(DISTINCT p.product_id) AS total FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id)";
  ...
  private function getOptionValuesCounterSQL($data) {
    ...
    if ($params) {
      ...
      foreach ($params as $option_id => $values) {
        if ($option_id == 'p' || $option_id == 'm' || $option_id == 'w' || $option_id == 's' || $this->isRange($values[0])) {
        	continue;
        }
