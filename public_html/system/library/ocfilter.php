...
final class OCFilter {
  ...
  public function getOptionsGet() {
	...
	// Вес товара
	$weight = $this->model_extension_module_ocfilter->getWeightByCategoryId($this->getCategoryId());
	if ($weight) {
		$options['weight'] = array(
			'option_id'   => 'w',
			'name'        => 'Вес товара',
			'description' => 'Описание веса товара',
			'type'        => $this->config->get('module_ocfilter_manufacturer_type'),
			'values'      => $weight
		);
	}
          ...
          $count = 0;
			...
			if ($count || !$this->config->get('module_ocfilter_hide_empty_values')) {
				...
				$values[] = array(
					...
					'name'     => html_entity_decode((isset($value['name']) ? $value['name'] : $this->weight->format($value['weight'], $value['weight_class_id'])) . (isset($option['postfix']) ? $option['postfix'] : ''), ENT_QUOTES, 'UTF-8'),
    ...
    if ($params) {
      foreach (explode($this->config->get('module_ocfilter_part_separator'), (string)$params) as $part) {
        ...
        if (isset($option[1])) {
          ...
          if ($this->isRange($option[1])) {
          ...
          } elseif ($option[0] == 'm' || $option[0] == 'w' || ($option[0] == 's' && $this->config->get('module_ocfilter_stock_status_method') == 'stock_status_id')) {
            ...
            if ($values) {
              $matches[] = $option[0] . $this->config->get('module_ocfilter_option_separator') . implode($this->config->get('module_ocfilter_option_value_separator'), $values);
  				...
  				$matches[] = 'w' . $this->config->get('module_ocfilter_option_separator') . $option[1];
