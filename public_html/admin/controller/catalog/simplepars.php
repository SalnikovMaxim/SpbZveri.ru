...
class ControllerCatalogSimplePars extends Controller
{
    ...
    public function phpscripts()
    {
        ...
        $data["mpage"] = $this->mPage();
        $data["setting"] = $this->model_catalog_simplepars->getSetting($data["dn_id"]);
        $data["scripts"] = $this->model_catalog_simplepars->scriptsFindAll();
                            ...
                            if ($this->request->get["who"] == "splitxml") {
                                ...
                                if ($this->request->get["who"] == "phpscripts") {
                                    if ($this->request->post["do"] == "get_script") {
                                        $value = $this->model_catalog_simplepars->scriptsGetData($this->request->post, $data["dn_id"]);
                                        exit(json_encode($value));
                                    }
                                    if ($this->request->post["do"] == "script_add_update") {
                                        $value = $this->model_catalog_simplepars->addOrUpdateScript($this->request->post, $data["dn_id"]);
                                        exit(json_encode($value));
                                    }
                                }
                            }
    ...
    public function mPage()
    {
        ...
        $mpage[4] = ["active" => "", "title" => "<i class=\"fa fa-cart-arrow-down\" aria-hidden=\"true\"></i> Парсинга в ИМ", "href" => $this->url->link("catalog/simplepars/productsetup", $adap["token"] . "&dn_id=" . $data["dn_id"], true)];
        $mpage[5] = ["active" => "", "title" => "<i class=\"fa fa-file-text\" aria-hidden=\"true\"></i> CSV/Парсинг", "href" => $this->url->link("catalog/simplepars/createcsv", $adap["token"] . "&dn_id=" . $data["dn_id"], true)];
        ...
        $mpage[11] = ["active" => "", "title" => "<i class=\"fa fa-exchange\" aria-hidden=\"true\"></i> Импорт/Экспорт Настроек", "href" => $this->url->link("catalog/simplepars/share", $adap["token"] . "&dn_id=" . $data["dn_id"], true)];
    ...
    public function actModule($data)
    {
        ...
        if (!empty($data["hash"])) {
            if ($data["hash"] == md5(sha1(sha1(sha1($_SERVER['HTTP_HOST'].'simplepars'.'4_5_1'))))) {
