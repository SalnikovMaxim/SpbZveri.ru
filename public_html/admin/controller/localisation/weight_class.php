...
class ControllerLocalisationWeightClass extends Controller {
	...
	protected function getList() {
		...
		$data['sort_cols'] = $this->url->link('localisation/weight_class', 'user_token=' . $this->session->data['user_token'] . '&sort=cols' . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->error['cols'])) {
			$data['error_cols'] = $this->error['cols'];
		} else {
			$data['error_cols'] = array();
		}
		...
		if (isset($this->request->post['cols'])) {
			...
			$data['cols'] = $weight_class_info['cols'];
		} else {
			$data['cols'] = '';
		}
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['weight_class_description'] as $language_id => $value) {
			...
			if (!$value['cols'] || (utf8_strlen($value['cols']) > 6)) {
				$this->error['cols'][$language_id] = 'Введите значение';
			}
