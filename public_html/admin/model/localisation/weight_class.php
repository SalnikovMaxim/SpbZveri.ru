...
class ModelLocalisationWeightClass extends Model {
	public function addWeightClass($data) {
		...
		foreach ($data['weight_class_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "weight_class_description SET weight_class_id = '" . (int)$weight_class_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', cols = '" . (int)$this->db->escape($value['cols']) . "', unit = '" . $this->db->escape($value['unit']) . "'");
		}
	...
	public function getWeightClassDescriptionByCols($cols) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "weight_class_description WHERE cols = '" . $this->db->escape($cols) . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");
