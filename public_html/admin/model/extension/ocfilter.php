...
class ModelExtensionOCFilter extends Model {
  ...
  public function copyFilters($data = array()) {
    ...
    if (!empty($data['copy_attribute'])) {
      $this->db->query("UPDATE " . DB_PREFIX . "product_attribute SET filter = TRIM(REPLACE(REPLACE(REPLACE(filter, '\t', ''), '\n', ''), '\r', ''))");
      ...
      $this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_option_description       (option_id, language_id, name)           SELECT (attribute_id + '" . (int)$o . "'), language_id, name FROM " . DB_PREFIX . "attribute_description");
  		...
  		$this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_option_value             (option_id, value_id)                    SELECT (attribute_id + '" . (int)$o . "'), (CRC32(CONCAT(attribute_id, filter)) + '" . (int)$v . "') FROM " . DB_PREFIX . "product_attribute WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY attribute_id, filter");
  		$this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_option_value_description (option_id, value_id, language_id, name) SELECT (attribute_id + '" . (int)$o . "'), (CRC32(CONCAT(attribute_id, filter)) + '" . (int)$v . "'), language_id, filter FROM " . DB_PREFIX . "product_attribute WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY attribute_id, filter");
      ...
      $this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_option_value_to_product  (option_id, value_id, product_id)        SELECT (attribute_id + '" . (int)$o . "'), (CRC32(CONCAT(attribute_id, filter)) + '" . (int)$v . "'), product_id FROM " . DB_PREFIX . "product_attribute WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");
      ...
      foreach ($languages as $language) {
        if ($language['language_id'] != $this->config->get('config_language_id')) {
          ...
          SELECT
            ...
            (SELECT
              (CRC32(CONCAT(pa2.attribute_id, pa2.filter)) + '" . (int)$v . "') FROM " . DB_PREFIX . "product_attribute pa2
            ...
            '" . (int)$language['language_id'] . "', pa.filter
          FROM " . DB_PREFIX . "product_attribute pa WHERE pa.language_id = '" . (int)$language['language_id'] . "' GROUP BY pa.attribute_id, pa.filter");
        }
      }
