...
//Для демки.
/*public function demoacc(){
	$file = DIR_APPLICATION.'simplepars/user_add.txt';
	$time_now = time();
	...
	if(empty($users[$_SERVER['REMOTE_ADDR']])){

		$users[$_SERVER['REMOTE_ADDR']] = $time_now;
		$users = json_encode($users);
		file_put_contents($file, $users);

	}elseif( !empty($users[$_SERVER['REMOTE_ADDR']]) && ( ($time_now-$users[$_SERVER['REMOTE_ADDR']]) > 86400 ) ){
		...
		exit("На демо сайте с одного аккаунта можно создавать не больше одного проекта в сутки. <br> Это временное ограничение.");
	}
}*/
...
public function ParsLink($data, $setting, $dn_id){
	...
	#$pre_html = str_replace(["href='#' link=\"", 'href="#" link="'], 'href="', $pre_html);
...
public function madeOption($data){
	...
	$data['opt_imgs'] = str_replace('{csvnc}', '{!na!}', $data['opt_imgs']);
	$data['opt_imgs'] = str_replace('{!na!}{!na!}', '{!na!}', $data['opt_imgs']);
...
public function optModuleOptionImegPro($pr_id, $product_option_id, $pr_opt_value_id, $value, $do){
	...
	if($do == 'add'){
		...
		#$this->wtfarrey('Обновление');
		$this->db->query("UPDATE ".DB_PREFIX."poip_option_image SET
			product_id = ".(int)$pr_id.",
			product_option_id = ".(int)$product_option_id.",
			image = '".$this->db->escape($value['imgs'])."',
			sort_order = 1
...
public function dwImagToProduct($dn_id, $imgs, $dir, $img_name, $under, $browser, $is_des=0){
	...
	foreach($imgs_chunk as $chunk){
		...
		foreach($data_img as $key => $img){
			...
			#$img_temp = mb_strtolower($img_temp);
...
public function preparinDataToStore($data, $dn_id){
	...
	$form['jan'] = $this->madeLogicalMathem($form['jan'], 'str');
	
	$form['isbn'] = trim(str_replace('{csvnc}','',$form['isbn']));
	$form['isbn'] = $this->madeLogicalMathem($form['isbn'], 'str');
	
	$form['mpn'] = trim(str_replace('{csvnc}','',$form['mpn']));
	$form['mpn'] = $this->madeLogicalMathem($form['mpn'], 'str');
...
public function toolWorkWithParentCat($data, $prs_id, $dn_id){
	...
	if(!empty($cats_now)){
		...
		if($data['operator'] == 1){
			...
			#$this->wtfarrey($cats_now);
			#$this->wtfarrey($cats_tmp);
			#$this->wtfarrey($cats_sql);
...
public function toolConvertWebpToJpeg($data, $prs_id, $dn_id){
	...
	if($data['operator'] == 1 && function_exists('imagecreatefromwebp') && function_exists('imagejpeg')){
...
public function getParamShow($data, $param_id, $dn_id){
	...
	#$delim = $this->db->query("SELECT * FROM `". DB_PREFIX ."pars_param` WHERE id=".(int)$param_id)->row;
...
public function addOrUpdateScript($data, $dn_id){
	$answer = [];
	...
	if(empty($data['file_name'])){
 	...
 	file_put_contents($file_path, $data['code']);
...
public function scriptsFindAll(){
...
public function scriptsGetData($data){
	#$this->wtfarrey($data);
	$script = ['name'=>'', 'code'=>''];
...
public function scriptGetAllTask($dn_id){
	...
	$tasks = $this->db->query("SELECT * FROM ".DB_PREFIX."pars_phpscripts WHERE dn_id =".(int)$dn_id)->rows;
	return $tasks;
}
...
public function urlEncoding($url){
  ...
  $restored = strtr($restored, ['%3D'=>'=','%3A'=>':','%2C'=>',','%2B'=>'+','&amp;'=>'&']);// Ибо rawurlencode заменяет равенство '=' на '%3D'
...
//фунция очистки страки от кавычек, перед записью в бд.
public function sqlstr($value){
