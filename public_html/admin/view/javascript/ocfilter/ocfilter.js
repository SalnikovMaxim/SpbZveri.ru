        ...
        if (json.message) {
          ...
          if (option.type == 'slide' || option.type == 'slide_dual') {
	            ...
	            if (!selecteds.length) selecteds = ['<b>' + ocfilter.php.text_select + '</b>'];
						...
						html.push('<div class="switcher" onclick="SelectedInp('+i+')"><div class="selected">' + selecteds.join('') + '</div><div class="values values-'+i+'"><input type="text" id="SelectedInp" placeholder="Поиск" class="form-control"><div class="valuesdd">' + values.join('') + '</div><div id="exits"></div></div>');
	          }
					}
...
function SelectedInp(id) {
 $(".values-"+id+" #SelectedInp").on("keyup", function() {  //2
    var value = $(this).val().toLowerCase();  //3
    $(".values-"+id+" .valuesdd div").filter(function() {  //4
